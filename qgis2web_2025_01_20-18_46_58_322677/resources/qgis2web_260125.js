...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    ...
    // Create a mapping of field names to static text
    var staticTextMap = {
        'OR_data': 'OR',
        'FR_data': 'FR',
        'WOR_data': 'WOR',
        'WFR_data': 'WFR',
        // Add more fields and their corresponding static text here
    };

    // Define the desired custom order of fields
    var customFieldOrder = [
        'name', 'dist_climb', 'descrip', 'OR_data', 'FR_data', 'WOR_data', 'WFR_data', 'link'
    ];

    // Filter and reorder the keys based on customFieldOrder
    var orderedKeys = customFieldOrder.filter(function(key) {
        return currentFeatureKeys.includes(key); // Include only keys that exist in currentFeatureKeys
    });

    // Loop through the reordered keys
    for (var i = 0; i < orderedKeys.length; i++) {
        var field = orderedKeys[i];
        if (field != 'geometry' && !['name', 'dist_climb', 'descrip', 'link'].includes(field)) { // Skip 'name', 'dist_climb', 'descrip' and 'link'
            var popupField = '';

            // Skip hidden fields
            if (layer.get('fieldLabels')[field] == "hidden field") {
                continue;
            }

            // Handle inline label visibility
            if (layer.get('fieldLabels')[field] == "inline label - visible with data") {
                if (currentFeature.get(field) == null) {
                    continue;
                }
            }

            // Handle the 3 fields without static text
            if (['name', 'dist_climb', 'descrip'].includes(field)) {
                var colspanValue = 2; // Span across both columns for these fields
                var fieldValue = currentFeature.get(field);
                popupText += '<tr><td colspan="' + colspanValue + '" style="text-align: center; font-weight: bold;">' +
                    (fieldValue != null ? fieldValue : 'No data') + '</td></tr>';
                continue; // Skip to the next field
            }

            // Format the popup field
            if (layer.get('fieldLabels')[field] == "inline label - always visible" ||
                layer.get('fieldLabels')[field] == "inline label - visible with data") {
                popupField += '<th>' + layer.get('fieldAliases')[field] + '</th><td>';
            } else {
                popupField += '<td colspan="2">';
            }

            // Add the field value (autolinker for clickable links)
            if (layer.get('fieldImages')[field] != "ExternalResource") {
                popupField += (currentFeature.get(field) != null 
                    ? autolinker.link(currentFeature.get(field).toLocaleString()) 
                    : 'No data') + '</td>';
            } else {
                // Handle images or videos
                var fieldValue = currentFeature.get(field);
                if (/\.(gif|jpg|jpeg|tif|tiff|png|avif|webp|svg)$/i.test(fieldValue)) {
                    ...
                    popupField += (fieldValue != null 
                        ? '<video controls><source src="images/' + fieldValue.replace(/[\\\/:]/g, '_').trim() + '" type="video/mp4">Your browser does not support video tags.</video>' 
                        : '') + '</td>';
                } else {
                    popupField += (fieldValue != null 
            ...
            // Add static text if the field is not 'name', 'descrip', or 'dist_climb'
            var staticText = '';
            if (!['name', 'descrip', 'dist_climb'].includes(field)) {
                staticText = staticTextMap[field] 
                    ? '<strong style="color: #124466; font-size: 12px;">' + staticTextMap[field] + ':</strong> ' 
                    : '<strong>Unknown Field:</strong> ';
            }

            // Combine static text and dynamic field into a single row
            popupText += '<tr><td colspan="2">' + staticText + popupField + '</td></tr>';
        }
    }
    return popupText;
}
            ...
            if (typeof clusteredFeatures !== "undefined") {
                if (doPopup) {
                    for (var n = 0; n < clusteredFeatures.length; n++) {
                        ...
                        popupText += '<c><b>' + currentFeature.get('dist_climb') + '</b></c>'; // Show field 'dist_climb' as subtitle in the popup
                        ...
                        <div class="tab-content" id="${uniqueId}-tab1" style="display:block;">
                            <table>
                                ${createPopupField(currentFeature, currentFeature.getKeys(), layer)}
                            </table>
                        </div>
                        <div class="tab-content" id="${uniqueId}-tab2" style="display:none;">
                            <p style="text-align: center; padding: 3px; color: #333; font-size: 12px;"> ${currentFeature.get('name')}.</p>
                        </div>
                        `;
    ...
    element: (() => {
        ...
        titleElement.innerHTML = `
            <div class="project-title-link" style="cursor: pointer;">
                <h2>SHR Long Distance Records</h2>
            </div>
        `;
...
var searchLayer = new SearchLayer({
    layer: lyr_LongDistanceRoutes_2,
    colName: 'name',
    zoom: 17,
    collapsed: true,
    map: map
